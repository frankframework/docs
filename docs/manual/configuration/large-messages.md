...
[`ForEachChildElementPipe`](https://reference.frankframework.org/#/components/ForEachChildElementPipe) defaults to Xalan (`xsltVersion=1`) rather than Saxon. This exception exists because `ForEachChildElementPipe` is designed to iterate over large messages and requires streaming support.
