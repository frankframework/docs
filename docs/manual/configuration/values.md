...
Custom session keys can be written with [`PutInSessionPipe`](https://reference.frankframework.org/#/components/PutInSessionPipe):
...
[`ReplacerPipe`](https://reference.frankframework.org/#/components/ReplacerPipe) substitutes both property references (`${...}`) and parameter references (`?{...}`) in its input. When the `find` attribute is omitted, no text replacement occurs — only property and parameter substitution.
...
[`EchoPipe`](https://reference.frankframework.org/#/components/EchoPipe) substitutes **properties only** (`${...}`). It does not substitute parameters (`?{...}`).
